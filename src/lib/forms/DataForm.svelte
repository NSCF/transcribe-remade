<script>
  ...
  */
  export let record = {
    projectID: null, batchNumber: null, specimenID: null, isCoreFields: true,
    barcodes: null, accessionNumber: null, primaryCollector: null, collectorNumber: null, 
    additionalCollectors: null, collectDay: null, collectMonth: null, collectYear: null, 
    currentName: null, typeName: null, qualifier: null, typeStatus: null, 
  ...
  let isQC = false
  let isType = false
  let originalIsCoreFields = false

  onMount(_=> {
    if(record.isCoreFields) {
      originalIsCoreFields = true
    }
  })
...
<div class="flex flex-col h-full">
  <div class="flex justify-between">
    <h1 class="text-xl">Specimen data</h1>
    {#if originalIsCoreFields}
      {#if record.isCoreFields}
        <button class="btn secondary " on:click={_ => record.isCoreFields = false}>Show hidden fields</button>
      {:else}
        <button class="btn secondary " on:click={_ => record.isCoreFields = true}>Hide extra fields</button>
      {/if}
    {/if}
  </div>
  <form class="w-full flex-1 min-h-0 p-1 pt-2 overflow-y-auto">
    <div class="relative mb-3">
      <input type="text" id="barcode" class="w-full peer placeholder-transparent" placeholder="barcode" bind:value={record.barcodes} />
      <label for="barcode" style="line-height: .8;" class="floating-label">barcode/s</label>
    </div>
    <div class="mb-3 w-full flex justify-between gap-2">
      
      <div class="relative" class:hidden={record.isCoreFields}>
        ...
        <label for="accession" style="line-height: .8;" class="floating-label">accession number</label>
      </div>
    ...
    <div class="mb-3 flex w-full gap-2" class:hidden={record.isCoreFields}>
    ...
    <div class="relative w-full mb-3" class:hidden={record.isCoreFields}>
    ...
    <div class="mb-3 max-w-full flex justify-between gap-4" class:hidden={record.isCoreFields}>
    ...
    <div class="relative w-full mb-3">
      ...
      <label for="currenttaxon" style="line-height: .8;" class="floating-label">latest det. name</label>
    </div>
    {#if isQC} <!--  change to record.typeStatus != null -->
      <div class="mb-3 w-full flex justify-between gap-4">
        <div class="relative flex-1">
          ...
          <label for="dettaxon" style="line-height: .8;" class="floating-label">type name</label>
        </div>
        <div class="relative w-1/3">
    ...
    {/if}
    <div class="mb-3 flex justify-between gap-2" class:hidden={record.isCoreFields}>
    ...
    <div class="mb-3 w-full flex justify-between gap-2" class:hidden={record.isCoreFields}>
      <div class="relative w-1/3">
    ...
    <div class="relative w-full mb-1" class:hidden={record.isCoreFields}>
    ...
    <div class="mb-3 max-w-full flex justify-between" class:hidden={record.isCoreFields}>
      <div class="relative w-1/4">
    ...
    <button class="btn primary ml-auto" on:click={emitRecord}>Save and next</button>
  </form>
</div>